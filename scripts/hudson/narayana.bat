...
rem BUILD BLACKTIE
call build.bat -f blacktie\pom.xml clean install "-DskipTests" "-Djbossas.ip.addr=%JBOSSAS_IP_ADDR%" || (call:fail_build & exit -1)

call build.bat -f blacktie\blacktie-admin-services\ear\pom.xml clean install "-Djbossas.ip.addr=%JBOSSAS_IP_ADDR%" || (call:fail_build & exit -1)

call build.bat -f blacktie\integration-tests\pom.xml clean test "-Dtest=CSTest" "-Djbossas.ip.addr=%JBOSSAS_IP_ADDR%" || (call:fail_build & exit -1)
