...
function init_test_options {
    is_ibm
    ISIBM=$?
    [ $NARAYANA_CURRENT_VERSION ] || NARAYANA_CURRENT_VERSION="5.3.6.Final-SNAPSHOT"
...
function build_as {
  ...
  comment_on_pull "$BUILD_URL using non-default AS_BRANCH: $AS_BRANCH"
...
function blacktie {
  ...
  unzip ${WORKSPACE}/blacktie/wildfly-blacktie/build/target/wildfly-blacktie-build-5.3.6.Final-SNAPSHOT-bin.zip -d $PWD/blacktie/wildfly-${WILDFLY_VERSION_FROM_JBOSS_AS}
  ...
  if [[ $# == 0 || $# > 0 && "$1" != "-DskipTests" ]]; then
    # START JBOSS
    if [ $JAVA_VERSION = "9-ea" ]; then
      JBOSS_HOME=`pwd`/blacktie/wildfly-${WILDFLY_VERSION_FROM_JBOSS_AS} JAVA_OPTS="-Xmx256m $JAVA_OPTS" blacktie/wildfly-${WILDFLY_VERSION_FROM_JBOSS_AS}/bin/standalone.sh -c standalone-blacktie.xml -Djboss.bind.address=$JBOSSAS_IP_ADDR -Djboss.bind.address.unsecure=$JBOSSAS_IP_ADDR -Djboss.bind.address.management=$JBOSSAS_IP_ADDR&
    else
